...
def expect_and_grad_ex(  
    ...
    #here we need to pass compilable info to the kernel function, so make lists 
    σ_list = []
    model_state_list = []
    pars_list = []
...
def grad_expect_hermitian_ex(
    ...
    #we need to store O_loc as E_loc in order to modify it afterwards and output cost function in the end
    E_loc  = O_loc
    ...
    #here we write a loop over the previously determined states, and use their information to modify E_loc and O_grad
    for i in range(len(shift_list)):
        #state_i = state_list[i]
        ...
        #psi_loc_1 refers to the one with varying state above
        ...
        #psi_loc_2 refers to the one with fixed state above
